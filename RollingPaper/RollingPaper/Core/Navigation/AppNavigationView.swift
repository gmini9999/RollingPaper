...
struct AppNavigationView: View {
    ...
    private func destination(for route: AppRoute) -> some View {
        ...
        case .home:
            HomeView(onCreatePaper: {
                coordinator.navigate(to: .paper(.create))
            }, onOpenPaper: { id in
    ...
    private func paperDestination(for route: PaperRoute) -> some View {
        switch route {
        case .create:
            PaperEditorView(paperID: nil) { id in
                coordinator.navigate(to: .paper(.share(id: id)))
            }
        case .detail(let id):
            PaperEditorView(paperID: id) { shareID in
    ...
    private func supplementaryColumn(for context: AdaptiveLayoutContext) -> some View {
        ScrollView {
            VStack(alignment: .leading, spacing: .rpSpaceM) {
                ...
                case .create:
                    Text("You are drafting a new paper. Use the detail column to compose content and the Share action to publish when ready.")
                        .font(.rpBodyM)
                ...
                case .share(let id):
                    ...
                    Text("Distribute the generated link or invite collaborators directly from this column.")
                        ...
                        .foregroundColor(.rpTextPrimary)
                case .none:
                    Text("Select a destination to see contextual tips and quick actions.")
                        .font(.rpBodyM)
                        .foregroundColor(.rpTextPrimary)
                }
